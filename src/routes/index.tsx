...
const SUBSCRIBE_FILE = "/home/team/shared/signups.jsonl";
...
const subscribeEmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const dir = dirname(SUBSCRIBE_FILE);
    if (!existsSync(dir)) {
    ...
    await appendFile(SUBSCRIBE_FILE, record + "\n");
    return { success: true };
  });
        ...
        <div className="mt-16 grid gap-8 lg:grid-cols-3">
          {tiers.map((tier) => (
            ...
            >
              ...
              >
                {tier.highlight ? "Get Early Access" : "Get Started"}
              </a>
            </div>
          ))}
        </div>
